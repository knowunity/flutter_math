...
class AccentUnderNode extends SlotableNode<EquationRowNode> {
  AccentUnderNode({required this.base, required this.label});
  ...
  BuildResult buildWidget(
    ...
    return BuildResult(
      ...
      widget: VList(
        children: <Widget>[
          VListElement(
            trailingMargin: label == '\u007e'
                ? 0.12.cssEm.toLpUnder(options)
                : 0.0,
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                if (label == '\u00AF') {
                  ...
                  return StrechySvgSpanWidget(
                    name: accentRenderConfig.underImageName!,
                    width: constraints.minWidth,
                    options: options,
                  );
                }
  ...
  Map<String, Object?> toJson() =>
      super.toJson()
        ..addAll({'base': base.toJson(), 'label': unicodeLiteral(label)});

  AccentUnderNode copyWith({EquationRowNode? base, String? label}) =>
      AccentUnderNode(base: base ?? this.base, label: label ?? this.label);
}
