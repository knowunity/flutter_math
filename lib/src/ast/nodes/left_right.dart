...
class LeftRightNode extends SlotableNode<EquationRowNode> {
  ...
  BuildResult buildWidget(
      ...
      if (index.isEven) {
        ...
        return LineElement(
          ...
          child: LayoutBuilderPreserveBaseline(
            builder: (context, constraints) => CustomSizedDelimWidget(
              delim: index == 0
                  ? leftDelim
                  : index == numElements - 1
                  ? rightDelim
    ...
    return BuildResult(
      options: options,
      widget: Line(children: childWidgets),
    );
  }
...
const delimiterShorfall = Measurement(value: 5, unit: Unit.pt);
...
class CustomSizedDelimWidget extends StatelessWidget {
  const CustomSizedDelimWidget({
    required this.delim,
    required this.minDelimiterHeight,
  ...
  Widget build(BuildContext context) {
    ...
    if (delimConf != null) {
      final axisHeight = options.fontMetrics.axisHeight.cssEm.toLpUnder(
        options,
      );
      return ShiftBaseline(
        relativePos: 0.5,
        ...
        child: MakeChar(
          character: delim!,
          font: delimConf.font,
          characterMetrics: lookupChar(delim!, delimConf.font, Mode.math),
          options: options,
        ),
      );
    } else {
      return StackedDelimWidget(
        delim: delim!,
        minDelimiterHeight: minDelimiterHeight,
        mode: Mode.math,
        options: options,
      );
    }
...
class StackedDelimWidget extends StatelessWidget {
  const StackedDelimWidget({
    required this.delim,
    required this.minDelimiterHeight,
    required this.mode,
    required this.options,
      ...
      child: Column(
        ...
        children: <Widget>[
          MakeChar(
            character: conf.top,
            font: conf.font,
            characterMetrics: topMetrics,
            options: options,
          ),
          for (var i = 0; i < repeatCount; i++)
            MakeChar(
              character: conf.repeat,
              font: conf.font,
              characterMetrics: repeatMetrics,
              options: options,
            ),
          if (conf.middle != null)
            MakeChar(
              character: conf.middle!,
              font: conf.font,
              characterMetrics: middleMetrics,
              options: options,
            ),
          ...
          MakeChar(
            character: conf.bottom,
            font: conf.font,
            characterMetrics: bottomMetrics,
            options: options,
          ),
        ],
      ),
